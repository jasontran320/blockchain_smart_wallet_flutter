...
contract ExpenseManagerContract{
    ...
    function deposit(uint _amount, string memory _reason) public payable{//payable is special condition that requires something?
        require(_amount >0, "Deposite should be greater than 0");//want a second condition
    ...
    function getBalance(address _account) public view returns (uint) {//view is key word to view
    ...
    function changeOwner(address _newOwner) public onlyOwner {//only owner keyword
        owner = _newOwner;
    }//modifiers can be applied to functions, are like functions
}